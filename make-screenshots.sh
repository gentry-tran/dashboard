...
render() {
  ...
  local body
  # TTLs pinned ENORMOUS for the whole run, not just seeded fresh once.
  #
  # Seeding the cache is not sufficient on its own: a full run is 25 themes at ~45s each, and
  # ACCOUNT_CACHE_TTL is 60s — so about a minute in, the seed goes stale, fetch_account fires,
  # and it overwrites the placeholder with the REAL account, which then appears in every
  # remaining screenshot. Location survived only because its TTL happens to be an hour. The
  # symptom is worse than a plain bug: the first images are clean and the later ones are not,
  # so a spot-check of one file says everything is fine.
  body=$(printf '%s' "$PAYLOAD" | DASHBOARD_THEME="$theme" DASHBOARD_TEMP_UNIT=F \
           DASHBOARD_CACHE_DIR="$TMPCACHE" \
           DASHBOARD_USAGE_TTL=99999999 DASHBOARD_LOCATION_TTL=99999999 \
           DASHBOARD_WEATHER_TTL=99999999 DASHBOARD_ACCOUNT_TTL=99999999 \
           ./dashboard.sh 2>/dev/null | ansi_to_html)
